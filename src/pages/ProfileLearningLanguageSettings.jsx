...
const ProfileLearningLanguageSettingsContainer = styled.div`
  min-height: 100vh;
  background: #ffffff;
  padding-left: 0;
...
const ProfileLearningLanguageSettings = () => {
  ...
  useEffect(() => {
    ...
    const savedLanguage = localStorage.getItem(
      "selectedProfileLearningLanguage"
    );
    if (savedLanguage) {
      ...
      if (currentLearningLanguage) {
        const languageObj = languages.find(
          (lang) => lang.name === currentLearningLanguage
        );
        if (languageObj) {
  ...
  const handleLanguageSelect = (languageCode) => {
    ...
    // 로컬 스토리지에 저장
    localStorage.setItem("selectedProfileLearningLanguage", languageCode);
...
export default ProfileLearningLanguageSettings;
